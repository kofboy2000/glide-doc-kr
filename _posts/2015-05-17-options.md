...
#### 썸네일 요청
Glide 의 [``thumbnail()``][9] API 는 [``RequestBuilder``][8] 를 특정하고, 메인 요청과 병렬로 실행 될 수 있게 해 줍니다. [``thumbnail()``][9] 은 메인 요청이 로딩되는 동안 나타날 수 있습니다. 만약 메인 요청이 썸네일 요청 전에 끝나게 될 경우, 썸네일 요청 건은 보이지 않게 됩니다. [``thumbnail()``][9] API 允许你简单快速地加载图像的低分辨率版本，并且同时加载图像的无损版本，这可以减少用户盯着加载指示器 *【例如进度条--译者注】* 的时间。
