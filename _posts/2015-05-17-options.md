...
결론적으로, ``Bitmap`` 에 대해서는 [DrawableTransitionOptions][7] 보다는 [BitmapTransitionOptions][6]  사용해야 하며, cross fade 를 사용하는 것 보다는 단순한 fade in 을 사욯나느 것이 좋습니다.
...
#### 缩略图 (Thumbnail) 请求
Glide 的 [``thumbnail()``][9] API 允许你指定一个 [``RequestBuilder``][8] 以与你的主请求并行启动。[``thumbnail()``][9] 会在主请求加载过程中展示。如果主请求在缩略图请求之前完成，则缩略图请求中的图像将不会被展示。[``thumbnail()``][9] API 允许你简单快速地加载图像的低分辨率版本，并且同时加载图像的无损版本，这可以减少用户盯着加载指示器 *【例如进度条--译者注】* 的时间。
