...
원문보기：[링크](http://bumptech.github.io/glide/doc/targets.html)

* TOC
{:toc}

### Target 이란
Glide 에서 [``Target``][1] 은 요청과 요청자 사이의 중간자 역할을 수행 합니다. Placeholder 를 표시하고, 리소스를 불러오며, 요청들에 알맞는 크기도 결정해 줍니다. 가장 자주 쓰이는 Target 은 [``ImageViewTargets``][2] 로 ImageView 를 사용해서 placeholder, Drawable 그리고 Bitmap 을 표시할 때 사용 됩니다. 사용자는 베이스 클래스를 상속 받아 하위 클래스를 만들거나 직접 Targets 를 구현할 수도 있습니다.

### 타켓 지정
[``into(Target)``][3] 함수는 요청을 시작할 뿐만 아니라 어떤 Target 이 요청을 받을지 지정하게 됩니다.
...
Glide 에서는 [``into(ImageView)``][4] 을 위해 요청된 리소스 타입이 적합한 ``ImageView`` 와 [``ImageViewTarget``][2] 에 적용될 수 있도록 도와주는 헬퍼(helper) 함수를 제공 합니다:
...
### 재사용 및 취소
[``into(Target)``][3] 와 [``into(ImageView)``][4] 은 ``Target`` 인스턴스를 리턴 합니다. 만약 이 ``Target`` 을 재사용 하신다면 기존에 시작되었던 요청은 취소가 되고 해당 요청의 리소스는 해제 됩니다.：
...
또한 리턴된 ``Target`` 을 새로운 로드를 시작하지 않고도 [``clear()``][14] 하고 연관된 리소스들을 해제 할 수 있습니다. ：
...
Glide 의 [``ViewTarget``][15] 하위 클래스는 Android Framework 의 [``getTag()``][8] 와 [``setTag()``][9] 함수를 사용해서 각 요청의 ``View`` 정보를 저장합니다. 따라서 [``ViewTarget``][15] 하위 클래스를 사용하거나 ``ImageView`` 에 로딩할 때 ``View`` 를 직접적으로 재사용 하거나 클리어(clear) 할 수 있습니다.:
...
추가적으로, **[``ViewTarget``][15] 에 한하여**，새로운 인스턴스를 각 로딩이나 클리어 할 때 전달 할 수 있습니다.：
